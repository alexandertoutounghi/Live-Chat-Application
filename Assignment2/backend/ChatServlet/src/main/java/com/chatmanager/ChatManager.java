...
public class ChatManager implements Serializable {
  ...
  public List<String> listMessageXml(ZonedDateTime from, ZonedDateTime to) {
    ...
    else if (messageTo.compareTo(list.get(0)) > 0) return new ArrayList<String>(); else {
      int fromIndex = list.indexOf(messageFrom);
      int toIndex = list.indexOf(messageTo);
      if (fromIndex == -1 && toIndex == -1) return new ArrayList<String>(); else if (
        fromIndex == -1
      ) result = list.subList(0, toIndex); else if (toIndex == -1) result =
        list.subList(fromIndex, list.size()); else result =
        list.subList(fromIndex, toIndex);
    }

    ArrayList<String> resultMessages = new ArrayList<>(list.size());
    for (Message m : result) resultMessages.add(m.toStringXml());
  ...
  public List<String> listMessages(ZonedDateTime from, ZonedDateTime to) {
    ...
    //if the earliest message is outside the range
    else if (messageTo.compareTo(list.get(0)) > 0) return new ArrayList<String>();
    //        else if (list.size() == 1) {
    ...
    ArrayList<String> resultMessages = new ArrayList<>(list.size());
    for (Message m : result) resultMessages.add(m.toString());
  ...
  public List<Message> clearMessages(ZonedDateTime from, ZonedDateTime to) {
    ...
    if (
      deleteFromIndex == -1 && deleteToIndex == -1
    ) return new ArrayList<Message>(); else if (deleteFromIndex == -1) list
      .subList(0, deleteToIndex)
      .clear(); else if (deleteToIndex == -1) list
      .subList(deleteFromIndex, list.size())
      .clear(); else list.subList(deleteFromIndex, deleteToIndex).clear();
